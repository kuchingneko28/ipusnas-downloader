...
async function shelfCommand(): Promise<void> {
  ...
  const picked: ShelfItem | null = await promptSelect(`Shelf (${books.length} books):`, [
    ...books.map((book) => ({
      value: book,
      label: `${book.book_title || book.book_id} — ${book.book_author || 'Unknown'} (returns ${(book.borrow_end_date || '').slice(0, 10)})`,
    })),
...
export function register(cli: CAC): void {
  cli.command('shelf', 'View borrowed books - select to download or return').action(async () => {
