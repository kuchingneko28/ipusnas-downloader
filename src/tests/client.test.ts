...
describe('API client', () => {
  ...
  it('rejects when no device PoP key is registered', async () => {
    session = { deviceId: 'd1', userToken: 'tok1' }; // no privatePem/publicJwk
    route = () => json({}, 200);

    await expect(getSecureBorrowKey('book-1', 'device-tok')).rejects.toThrow(/No device PoP key registered/);
    expect(requests).toHaveLength(0); // guard fires before any fetch
  });
